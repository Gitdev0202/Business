"""
Nintendo Switch 2 Marktplaats monitor.

Haalt nieuwe advertenties op uit de Marktplaats-categorie "Spelcomputers |
Nintendo Switch" (id 2943), filtert op vandaag geplaatst + minimumprijs,
sluit losse spellen en defecte advertenties uit, en stuurt alleen
nog-niet-eerder-geziene advertenties naar Discord via een webhook. Zelfde
aanpak als de PS5-/MacBook-monitor: gewoon een vaste minimumprijs, geen
marge-berekening. Combi's (Switch 2 + games/Pro Controller) tellen mee,
zolang de console zelf het hoofditem is.

Gezien-ids worden bijgehouden in state/seen_switch2.json zodat elke run
...
L2_CATEGORY_ID = 2943  # Spelcomputers | Nintendo Switch

SEARCH_URL = "https://www.marktplaats.nl/lrp/api/search"
PAGE_SIZE = 100
MAX_PAGES = 5

MIN_PRICE_EUR = float(os.environ.get("MIN_PRICE_EUR", "300"))
...
# Alleen Switch 2 -- niet OLED, niet Lite, niet origineel/V2.
TITLE_PATTERN = re.compile(r"switch\s*2\b", re.IGNORECASE)

BROKEN_KEYWORDS = [
    "voor onderdelen", "defect", "niet werkend", "kapot scherm",
    "start niet op", "gaat niet aan", "geen beeld",
]

# Attributen die alleen bij een los SPEL voorkomen, nooit bij een console
# (bv. "genre": "Actie", "numberOfPlayers": "2 spelers"). Marktplaats'
# l2CategoryId-parameter is soms geen harde filter -- af en toe glipt een
# los spel toch mee. Dit signaal pakt alleen echte spellen, zonder het
# risico dat een echte Switch 2 die toevallig in een net iets andere
# categorie staat wordt gemist.
GAME_ONLY_ATTRIBUTE_KEYS = {"genre", "numberOfPlayers", "multiplayerPossibilities"}

STATE_FILE = os.path.join(os.path.dirname(__file__), "..", "state", "seen_switch2.json")
PRUNE_AFTER_DAYS = 21
...
def is_excluded(listing: dict) -> bool:
    ...
    return _contains_any(haystack, BROKEN_KEYWORDS)
...
def is_relevant(listing: dict) -> bool:
    return (
        ...
        and not is_actual_game_not_console(listing)
        and not is_excluded(listing)
...
def build_embed(listing: dict) -> dict:
    ...
    embed = {
        "title": listing.get("title", "Nintendo Switch 2 advertentie")[:256],
...
def send_discord_notifications(listings: list) -> None:
    ...
    for i in range(0, len(listings), chunk_size):
        ...
        payload = {
            "username": "Nintendo Switch 2 Marktplaats Monitor",
            "content": (
                f"🎮 {len(chunk)} nieuwe Nintendo Switch 2-advertentie(s) gevonden!"
                if i == 0 else None
            ),
...
def main() -> int:
    ...
    print(
        f"Opgehaald: {len(listings)} advertenties uit categorie Nintendo Switch. "
